import simeck64128_rkdiff as sm
import numpy as np
import  math
...
def make_resnet(num_blocks=2, num_filters_1=64, num_filters_2=64, num_filters_3=64, num_outputs=1, d0=512,d1=64, d2=64, word_size=32, ks=3,depth=5, reg_param=0.0001, final_activation='sigmoid',s_groups = 1,ks_value_1=1,ks_value_2=3,ks_value_3=3,dropout_value=0.5,se_ratio=32):
...
def data_generator(bs,num_rounds,diff,key_diff,s_groups):
  while True:
    X, Y = sm.make_train_data(bs,num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
    yield X,Y
    
import os
# def train_distinguisher(diff,key_diff,wdir,curr_time_str,num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=2000,lr_choice="14",d0=512,d1=64,d2=64,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=64,num_filters_2=64,num_filters_3=64,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=32):
def train_distinguisher(diff, key_diff, wdir, curr_time_str, num_epochs, num_rounds, depth, filename, s_groups, bs, lr_choice, d0, d1, d2, ks_value_1, ks_value_2, ks_value_3, machine_name, dropout_value, num_filters_1, num_filters_2, num_filters_3, reg_param, high_lr, low_lr, lr_epoch, se_ratio):  
  
    import os
    import pandas as pd

    repeat_dir = wdir + curr_time_str + '/'
    if(not os.path.exists(repeat_dir)):
      os.makedirs(repeat_dir)

    excel_name = repeat_dir+"record.xlsx"    
    ...
    check = make_checkpoint(repeat_dir+'best'+str(num_rounds)+'depth'+str(depth)+'.h5')
    ...
    X, Y = sm.make_train_data_two(n_train, num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
    X_eval, Y_eval = sm.make_train_data_two(n_test, num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
    h = net.fit(X,Y,epochs=num_epochs,batch_size=bs, verbose=2, validation_data=(X_eval, Y_eval), callbacks=[lr,check])

    # X_eval, Y_eval = sm.make_train_data(n_test, num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
    # h = net.fit_generator(data_generator(bs,num_rounds,diff,key_diff,s_groups),  steps_per_epoch=n_train//bs,epochs=num_epochs, verbose=2, validation_data=(X_eval, Y_eval), callbacks=[lr,check])
      

    params = "ep{0}_round{1}_depth{2}_group{3}_bs{4}_lr{5}_d1{6}_d2{7}_ks{8}_dp{9}_filter{10}_reg{11}_lr{12}_{13}_lrepoch{14}_{15}".format(str(num_epochs),str(num_rounds),str(depth),str(s_groups),str(bs),str(lr_choice),str(d1),str(d2),str(ks_value_1)+str(ks_value_2)+str(ks_value_3),str(dropout_value)[2:],str(num_filters_1)+"-"+str(num_filters_2)+"-"+str(num_filters_3),str(reg_param),str(high_lr)+"-"+str(low_lr),machine_name,str(lr_epoch),curr_time_str)
    file = open(repeat_dir+"params.txt", "w")
    file.write(params)
    file.close()
    ...
    fig_name = repeat_dir+"train_history_acc.jpg"
    ...
    fig_name = repeat_dir + r"train_history_loss.jpg"
    plt.savefig(fig_name)
    ...
    old_df = pd.read_excel(excel_name)
    new_df = {
        # 'diff':[f"{hex(diff[0])}_{hex(diff[1])}"],
        # 'key_diff':[f"{hex(key_diff[0])}_{hex(key_diff[1])}_{hex(key_diff[2])}_{hex(key_diff[3])}"],
        'diff':[f"{hex(diff[0][0])}_{hex(diff[0][1])}_{hex(diff[1][0])}_{hex(diff[1][1])}"],
        'key_diff':[f"{hex(key_diff[0][0])}_{hex(key_diff[0][1])}_{hex(key_diff[0][2])}_{hex(key_diff[0][3])}_{hex(key_diff[1][0])}_{hex(key_diff[1][1])}_{hex(key_diff[1][2])}_{hex(key_diff[1][3])}"],
        'epoch':[num_epochs],
        'round': [num_rounds],
        'depth':[depth],
        'group':[s_groups],
