...
def train_distinguisher(diff,key_diff,wdir,curr_time_str,num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=2000,lr_choice="14",d0=512,d1=64,d2=64,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=32,num_filters_2=32,num_filters_3=32,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16):
    ...
    X, Y = sp.make_train_data_two(n_train,num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
    X_eval, Y_eval = sp.make_train_data_two(n_test, num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)

    # X, Y = sp.make_train_data(10**4*2,num_rounds,s_groups=s_groups,diff=diff,key_diff=key_diff)
    # X_eval, Y_eval = sp.make_train_data(10**4*2, num_rounds,s_groups=s_groups,diff=diff,key_diff=key_diff)
