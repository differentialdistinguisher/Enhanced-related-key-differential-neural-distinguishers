...
def pad_and_split_hex_string(hex_str, target_length,item_length):  
    """  
    ...
    # Split the padded hexadecimal value into item_length-digit chunks  
    hex_chunks = [padded_hex_value[i:i+item_length] for i in range(0, len(padded_hex_value), item_length)]  
...
if __name__=='__main__':
    ...
    parser.add_argument("--num_rounds",type=int,default=13)
    parser.add_argument("--depth_value",type=int,default=5)
    parser.add_argument("--file_name",type=str,default="default.xlsx")
    ...
    parser.add_argument("--se_ratio",type=int,default=32)
    args = parser.parse_args()
    
    # diff = pad_and_split_hex_string(args.diff,16,8)
    # key_diff = pad_and_split_hex_string(args.key_diff,32,8)
    # wdir = f"./{hex(diff[0])}_{hex(diff[1])}/{hex(key_diff[0])}_{hex(key_diff[1])}_{hex(key_diff[2])}_{hex(key_diff[3])}/"
    diffs = args.diff.split("_")
    diff0 = pad_and_split_hex_string(diffs[0],16,8)
    diff1 = pad_and_split_hex_string(diffs[1],16,8)
    diff = (diff0,diff1)
    
    key_diffs = args.key_diff.split("_")
    key_diff0 = pad_and_split_hex_string(key_diffs[0],32,8)
    key_diff1 = pad_and_split_hex_string(key_diffs[1],32,8)
