...
const = [0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffd, 0xfffffffd, 0xfffffffd, 0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffd, 0xfffffffc, 0xfffffffc, 0xfffffffc]
# print(len(const))
...
def expand_key_simon_reversed(k, t):
    ...
    for i in range(t - 4):
        ...
        # print(i,t-5-i)
        ks[i+4] = const[t-5-i] ^ ks[i] ^ tmp ^ ror(tmp,1)
...
def make_train_data_two(n, nr, diff=((0x0,0x40),(0x0,0x40)),key_diff=((0x0,0x0,0x0,0x0),(0x0,0x0,0x0,0x0)),s_groups=1):
    Y = np.frombuffer(urandom(n), dtype=np.uint8)
    Y = Y & 1
    
    keys = np.frombuffer(urandom(16 * n), dtype=np.uint32) 
    keys = keys.reshape(4, -1)
    keys_diff = np.array([keys[0] ^ key_diff[0][0], keys[1] ^ key_diff[0][1], keys[2] ^ key_diff[0][2], keys[3] ^ key_diff[0][3]],dtype=np.uint32)
    keys_diff[0][Y==0] = keys[0][Y==0] ^ key_diff[1][0]
    keys_diff[1][Y==0] = keys[1][Y==0] ^ key_diff[1][1]
    keys_diff[2][Y==0] = keys[2][Y==0] ^ key_diff[1][2]
    keys_diff[3][Y==0] = keys[3][Y==0] ^ key_diff[1][3]
    ...
    for i in range(s_groups):
        ...
        plain1l = plain0l ^ diff[0][0]
        plain1r = plain0r ^ diff[0][1]
        ...
        plain1l[Y == 0] = plain0l[Y == 0] ^ diff[1][0]
        plain1r[Y == 0] = plain0r[Y == 0] ^ diff[1][1]
...
def getdiffs_reversed(nr, n=10**4, diff=(0x0,0x40),key_diff=(0x0,0x0,0x0,0x0)):
    ...
    ctdata0l = np.frombuffer(urandom(4 * n), dtype=np.uint32)
    ctdata0r = np.frombuffer(urandom(4 * n), dtype=np.uint32)
...
def check():
    ...
    keys_reversed = np.array([[366954134],[203224979],[1906196252],[404331522]])
    ks_new = expand_key_simon_reversed(keys_reversed,nr)
    # print(ks_new)
    assert np.all(ks_new == ks)
