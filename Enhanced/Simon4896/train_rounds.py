...
if __name__=='__main__':
    ...
    parser.add_argument("--se_ratio",type=int,default=24)
    args = parser.parse_args()
    
    # diff = pad_and_split_hex_string(args.diff,12,6)
    # key_diff = pad_and_split_hex_string(args.key_diff,24,6)
    # wdir = f"./{hex(diff[0])}_{hex(diff[1])}/{hex(key_diff[0])}_{hex(key_diff[1])}_{hex(key_diff[2])}_{hex(key_diff[3])}/"
    
    diffs = args.diff.split("_")
    diff0 = pad_and_split_hex_string(diffs[0],12,6)
    diff1 = pad_and_split_hex_string(diffs[1],12,6)
    diff = (diff0,diff1)
    
    key_diffs = args.key_diff.split("_")
    key_diff0 = pad_and_split_hex_string(key_diffs[0],24,6)
    key_diff1 = pad_and_split_hex_string(key_diffs[1],24,6)
