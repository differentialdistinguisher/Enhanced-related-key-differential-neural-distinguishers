...
def data_generator(repeat_dir,n_train,bs):
  while True:
    for id in range(n_train//bs+1):
      X = np.load(f"{repeat_dir}X_{id}.npy")
      Y = np.load(f"{repeat_dir}Y_{id}.npy")
      yield X,Y
...
def train_distinguisher(diff,key_diff,wdir,curr_time_str,num_epochs, num_rounds=7, depth=1,filename="default",s_groups=1,bs=2000,lr_choice="14",d0=512,d1=64,d2=64,ks_value_1=1,ks_value_2=3,ks_value_3=3,machine_name='vgc',dropout_value=0.8,num_filters_1=32,num_filters_2=32,num_filters_3=32,reg_param=10**-4,high_lr=0.002,low_lr=0.0001,lr_epoch=10,se_ratio=16):
    ...
    if s_groups < 64:
      ...
      for id in range(n_train//bs+1):
        print(id,n_train//bs+1)
        if os.path.exists(f"{repeat_dir}X_{id}.npy") and os.path.exists(f"{repeat_dir}Y_{id}.npy"):
          continue
        X, Y = sm.make_train_data(bs,num_rounds,diff=diff,key_diff=key_diff,s_groups=s_groups)
        np.save(f"{repeat_dir}X_{id}",X) 
        np.save(f"{repeat_dir}Y_{id}",Y) 

      h = net.fit_generator(data_generator(repeat_dir,n_train,bs),  steps_per_epoch=n_train//bs+1,epochs=num_epochs, verbose=2, validation_data=(X_eval, Y_eval), callbacks=[lr,check])
      for id in range(n_train//bs+1):
        os.remove(f"{repeat_dir}X_{id}")
        os.remove(f"{repeat_dir}Y_{id}")
